    ...
    function listAvailablePrinters()
    {
        ...
        foreach ($list['available'] as $printer_det)
        {
            ...
            if ($conf->global->PRINTING_GCP_DEFAULT == $printer_det['id'])
            {
                ...
                $html.= '<a href="'.$_SERVER["PHP_SELF"].'?action=setvalue&amp;mode=test&amp;varname=PRINTING_GCP_DEFAULT&amp;driver=printgcp&amp;value='.urlencode($printer_det['id']).'" alt="'.$langs->trans("Default").'">'.'<i class="fa fa-toggle-off" title="Disabled"></i>'.'</a>';
            $html.= '</td>';
            $html.= '</tr>'."\n";
        }
