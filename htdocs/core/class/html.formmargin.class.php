...
class FormMargin
{
	...
	function displayMarginInfos($object, $force_price=false)
	{
		...
		if (! empty($conf->global->MARGIN_ADD_SHOWHIDE_BUTTON))	// TODO Warning this feature rely on an external js file that may be removed. Using native js function document.cookie should be better
		{
	    	...
	    	print '<span id="showMarginInfos" class="linkobject '.(!empty($hidemargininfos)?'':'hideobject').'">'.'<i class="fa fa-toggle-off" title="Disabled"></i>'.'</span>';
	    	print '<span id="hideMarginInfos" class="linkobject '.(!empty($hidemargininfos)?'hideobject':'').'">'.'<i class="fa fa-toggle-on" title="Enabled"></i>'.'</span>';
